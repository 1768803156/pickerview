...
//                maxItem = endD;
     ...
     */
    public void setCyclic(boolean[] cyclic) {
        if (cyclic == null || cyclic.length < 6) return;
        wv_year.setCyclic(cyclic[0]);
        wv_month.setCyclic(cyclic[1]);
        wv_day.setCyclic(cyclic[2]);
        wv_hours.setCyclic(cyclic[3]);
        wv_minutes.setCyclic(cyclic[4]);
        wv_seconds.setCyclic(cyclic[5]);
    }
    ...
    public void setVisibleItem(int itemsVisible) {
        ...
        wv_minutes.setItemsVisible(itemsVisible);
        wv_seconds.setItemsVisible(itemsVisible);
    }
}
